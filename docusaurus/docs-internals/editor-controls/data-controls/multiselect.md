...
| `config?.dependencies` | `Array<unknown>`                                                                                                                                                                           |      -       | This option is effective only when `config.fetchOnMount` is set to `true`. The dependencies parameter must be an array of values. When any value in this array changes, the control triggers a data refetch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
#### Config `dependencies` example

This configuration is primarily useful when the `multiSelect` depends on external values, for example when products need to be refetched based on the selected category.

```js
const getToolbarItems = ({ getValue }) => {
  const categories = getValue("categories");

  const loadCategories = async (value: Value, abortSignal: AbortSignal): Promise<Choice[]> => {};

  const searchCategories = async (search: string, abortSignal: AbortSignal): Promise<Choice[]> => {};

  const loadProducts = async (value: Value, abortSignal: AbortSignal): Promise<Choice[]> => {
    const params = new URLSearchParams({ categories });
    const response = await fetch(`${URL}?${params}`, { signal: abortSignal });

    return response.json();
  };

  const searchProducts = async (search: string, abortSignal: AbortSignal): Promise<Choice[]> => {
    const params = new URLSearchParams({ categories });
    const body = JSON.stringify({ searchCriteria: search });

    const response = await fetch(`${URL}?${params}`, { signal: abortSignal, body });
