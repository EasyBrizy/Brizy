  ...
  style: ({ value }) => {
    return {
      "{{WRAPPER}}:hover .brz-text": {
        ...
        "text-shadow": value.paletteVar
          ? `${value.horizontal}px ${value.vertical}px ${value.blur}px rgba(var(${value.paletteVar}), ${value.opacity})`
          : `${value.horizontal}px ${value.vertical}px ${value.blur}px ${value.hex}`
      }
    }
  }
}
